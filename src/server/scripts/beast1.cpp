...
void CreatePet(Player *player, Creature * m_creature, uint32 entry) {
        ...
        if(player->getClass() != CLASS_HUNTER) {
            player->GetSession()->SendNotification("You are not a Hunter!", player->GetGUID());
            player->PlayerTalkClass->CloseGossip();
            return;
        }
 
        if(player->GetPet()) {
            player->GetSession()->SendNotification("First you must drop your Pet!", player->GetGUID());
            player->PlayerTalkClass->CloseGossip();
            return;
        }
        ...
        sLog->outString("Pet Create fail: no init stats for entry %u", entry);
        ...
        //end
        player->PlayerTalkClass->CloseGossip();
        player->GetSession()->SendNotification("Pet added. You might want to feed it and name it somehow.", player->GetGUID());
    }
